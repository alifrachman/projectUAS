import 'package:farm_indo/models/article.dart';

import 'models/product.dart';

final List<Product> dataProducts = [
  Product(
    imageUrl:
        "https://megaeltra.com/wp-content/uploads/2020/02/1539575734-932-583504.jpg",
    title: "Pupuk",
    desc:
        'Pupuk adalah material yang ditambahkan pada media tanam atau tanaman untuk mecukupi kebutuhan hara yang diperlukan sehingga mampu berproduksi dengan baik. Material pupuk dapat berupa bahan organik ataupin non-organik',
    price: 80000,
    qty: 1,
  ),
  Product(
    imageUrl:
        "https://www.tokotrubus.co.id/wp-content/uploads/2020/09/Benih-Cabe-Rawit-SRET.jpg",
    title: "Bibit Cabe",
    desc: '-',
    price: 20000,
    qty: 1,
  ),
  Product(
    imageUrl:
        "https://www.ptppi.co.id/wp-content/uploads/2020/11/bg-dharmabas-300x249.png",
    title: "Pestisida",
    desc: '-',
    price: 500000,
    qty: 1,
  ),
  Product(
    imageUrl:
        "https://s3.bukalapak.com/img/8790023386/large/SWAN_HAND_SPRAYER_16_L_MANUAL_ALAT_SEMPROT_HAMA_MTB_16.jpg",
    title: "Alat Semprotan",
    desc: '-',
    price: 200000,
    qty: 1,
  ),
];

final List<Article> dataArticles = [
  Article(
    imageUrl:
        "https://ik.imagekit.io/carro/jualo/original/29446740/image_20210129-1581-759juo.jpg?v=1611891127",
    title: "Rekomendasi pupuk untuk tanaman",
  ),
  Article(
    imageUrl:
        "https://ds393qgzrxwzn.cloudfront.net/resize/c500x500/cat1/img/images/0/p2G2FSuZTY.jpg",
    title: "Temukan kebutuhanmu di FARM INDO",
  ),
];
